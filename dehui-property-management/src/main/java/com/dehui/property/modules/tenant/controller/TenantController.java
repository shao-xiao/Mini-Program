...
public class TenantController {
    ...
    public ApiResponse<Void> saveTenant(@RequestBody Map<String, Object> body) {
        ...
        try {
            if (id == null) {
                jdbcTemplate.update(
                        """
                        INSERT INTO tenant (code, name, credit_code, contact_phone, status, created_by, updated_by, remark)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                        """,
                        JdbcMaps.code("TEN"),
                        tenantName,
                        JdbcMaps.str(body, "businessLicense", "creditCode"),
                        JdbcMaps.str(body, "contactPhone", "phone"),
                        status,
                        0L,
                        0L,
                        JdbcMaps.str(body, "contactPerson", "contactEmail", "remark")
                );
            } else {
                int updated = jdbcTemplate.update(
                        """
                        UPDATE tenant
                        SET name = ?, credit_code = ?, contact_phone = ?, status = ?, updated_by = ?, remark = ?
    ...
    public ApiResponse<Map<String, Object>> saveTenantContact(@PathVariable Long tenantId, @RequestBody Map<String, Object> body) {
        ...
        String phone = JdbcMaps.requiredStr(body, "Contact phone is required", "phone");
        ...
        if (id == null) {
            jdbcTemplate.update(
                    """
                    INSERT INTO tenant_contact (code, tenant_id, name, phone, role, is_primary, status, created_by, updated_by)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    JdbcMaps.code("TC"),
                    tenantId,
            ...
            int updated = jdbcTemplate.update(
                    ...
                    SET name = ?, phone = ?, role = ?, is_primary = ?, status = ?, updated_by = ?
    ...
    public ApiResponse<Map<String, Object>> tenantOverview(@PathVariable Long tenantId) {
        Map<String, Object> tenant = jdbcTemplate.queryForMap(
                """
                SELECT id, code AS tenantCode, name AS tenantName, contact_phone AS contactPhone,
